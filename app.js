...
const state = {
  ...
  progress: {
    1: { box: 2, due: true, correct: 8, wrong: 1, lastResult: "correct" },
    2: { box: 1, due: true, correct: 2, wrong: 3, lastResult: "wrong" },
    3: { box: 0, due: false, correct: 0, wrong: 0, lastResult: "" },
    4: { box: 3, due: true, correct: 11, wrong: 2, lastResult: "correct" },
  },
  filters: {
    q: "",
    level: "all",
...
function render() {
  app.innerHTML = `
    <div class="shell ${state.mode === "learner" ? "learner-shell" : ""}">
      ${state.mode === "learner" ? renderLearnerSidebar() : renderSidebar()}
      <main class="main">
        ${state.mode === "learner" ? renderLearnerTopbar() : renderTopbar()}
        <section class="content">${state.mode === "learner" ? renderLearnerView() : renderView()}</section>
      </main>
    </div>
  ...
  return `
    <aside class="sidebar learner-sidebar">
      ...
      <div class="sidebar-footer">
        ...
        <button class="btn primary" data-mode="admin">进入后台</button>
      </div>
    </aside>
  `;
}
  ...
  return `
    <aside class="sidebar">
      ...
      <div class="sidebar-footer">
        ...
        <button class="btn primary" data-mode="learner">返回学习端</button>
      </div>
    </aside>
  `;
}
  ...
  return `
    <header class="topbar">
      ...
      <div class="top-actions">
        ...
        <button class="btn" data-mode="admin">后台管理</button>
      </div>
    </header>
  `;
}
    ...
    <div class="study-layout">
      <div class="study-card panel">
        ...
        <div class="answer-panel ${state.reviewRevealed ? "revealed" : ""}">
          ...
          <div class="tag-row">${current.tags.map((tag) => `<span>${tag}</span>`).join("")}</div>
        </div>
    ...
    <div class="grid-2">
      ...
      <div class="panel">
        ...
        <div class="panel-body">
          <div class="table-wrap">${wordTable(state.words.filter((word) => word.status === "published"))}</div>
        </div>
      </div>
    </div>
  `;
...
function wordTable(words) {
  const actionHeader = state.mode === "learner" ? "练习" : "操作";
  return `
    <table>
      <thead><tr><th>单词</th><th>释义</th><th>等级/词性</th><th>例句</th><th>状态</th><th>${actionHeader}</th></tr></thead>
      <tbody>
        ${words.map((word) => `
          <tr>
            <td><div class="jp">${word.japanese}</div><div class="kana">${word.kana}</div></td>
