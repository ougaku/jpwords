...
第一阶段先做一个可独立发布的移动端 App：无需登录，内置固定词库，本地保存学习进度。免费版展示广告并开放基础词库；付费版移除广告并解锁更多内置词库。后台管理系统暂时继续作为内容维护与商业化规则原型，不进入第一版移动端运行链路。
...
- 商业化：免费版展示广告；付费状态存在本地，后续接入应用商店内购校验。
...
- 错词强化：错题自动加入短间隔复习。
- 广告位置：学习结束页、词库列表底部、统计页底部；复习卡片中不插入广告，避免打断核心体验。
...
- 内购与广告：先保留 Provider 接口和 UI 占位，接入时替换实现。
